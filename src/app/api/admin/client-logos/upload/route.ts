...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const logoId = form.get("logoId") as string | null;
    const file = form.get("file") as File | null;
    if (!logoId || !file) return NextResponse.json({ error: "logoId y file requeridos" }, { status: 400 });
    if (!ALLOWED.includes(file.type)) return NextResponse.json({ error: "Tipo de archivo no permitido" }, { status: 400 });
    const ext = file.name.split(".").pop()?.toLowerCase() ?? "png";
    const path = `logo-${logoId}.${ext}`;
    const arrayBuffer = await file.arrayBuffer();
    const publicUrl = await supaUploadFile(BUCKET, path, arrayBuffer, file.type);
    const { error } = await supaUpdate("client_logos", { url: publicUrl }, "id", logoId);
